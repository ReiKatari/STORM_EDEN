...
fs::path GetAppDataRoamingDirectory() {
    ...
    HRESULT hr = SHGetKnownFolderPath(FOLDERID_RoamingAppData, 0, nullptr, &appdata_roaming_path);

    fs::path fs_appdata_roaming_path;
    if (SUCCEEDED(hr) && appdata_roaming_path != nullptr) {
        fs_appdata_roaming_path = fs::path{appdata_roaming_path};
        CoTaskMemFree(appdata_roaming_path);
    } else {
        LOG_ERROR(Common_Filesystem, "Failed to get the path to the %APPDATA% directory, hr=0x{:X}", (unsigned int)hr);
        // Fallback to environment variable
        const wchar_t* appdata_env = _wgetenv(L"APPDATA");
        if (appdata_env != nullptr) {
            fs_appdata_roaming_path = fs::path{appdata_env};
        }
    }
