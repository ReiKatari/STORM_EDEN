...
struct System::Impl {
    ...
    bool SaveState(const std::string& filepath) {
        ...
        LOG_INFO(Core, "Saving state to {}", filepath);
        Common::FS::IOFile file{filepath, Common::FS::FileAccessMode::Write, Common::FS::FileType::BinaryFile};
    ...
    bool LoadState(const std::string& filepath) {
        ...
        LOG_INFO(Core, "Loading state from {}", filepath);
        Common::FS::IOFile file{filepath, Common::FS::FileAccessMode::Read, Common::FS::FileType::BinaryFile};
        if (!file.IsOpen()) return false;
    ...
    DummyLogger log1{"DummyLog: After profile_manager"};
    /// Network instance
    Network::NetworkInstance network_instance;
    DummyLogger log2{"DummyLog: After network_instance"};
    Core::SpeedLimiter speed_limiter;
    DummyLogger log3{"DummyLog: After speed_limiter"};
    ExecuteProgramCallback execute_program_callback;
    DummyLogger log4{"DummyLog: After execute_program_callback"};
    ExitCallback exit_callback;
    DummyLogger log5{"DummyLog: After exit_callback"};
    ...
    DummyLogger log6{"DummyLog: After gpu_dirty_memory_managers"};
    ...
    DummyLogger log7{"DummyLog: After service_manager"};
    /// ContentProviderUnion instance
    std::unique_ptr<FileSys::ContentProviderUnion> content_provider;
    DummyLogger log8{"DummyLog: After content_provider"};
    /// AppLoader used to load the current executing application
    std::unique_ptr<Loader::AppLoader> app_loader;
    DummyLogger log9{"DummyLog: After app_loader"};
    std::unique_ptr<Tegra::GPU> gpu_core;
    DummyLogger log10{"DummyLog: After gpu_core"};
    std::stop_source stop_event;
    DummyLogger log11{"DummyLog: After stop_event"};
...
bool System::SaveState(const std::string& filepath) {
    return impl->SaveState(filepath);
}

bool System::LoadState(const std::string& filepath) {
    return impl->LoadState(filepath);
}
...
u64 System::GetApplicationProcessProgramID() const {
    auto* process = impl->kernel.ApplicationProcess();
    return process ? process->GetProgramId() : 0;
}
