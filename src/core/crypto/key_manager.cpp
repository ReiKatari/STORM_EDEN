...
void KeyManager::ReloadKeys() {
    ...
    std::vector<std::filesystem::path> candidate_dirs = {
        keys_dir,
        Common::FS::GetLegacyPath(Common::FS::EmuPath::StormEdenDir) / "keys",
        Common::FS::GetLegacyPath(Common::FS::EmuPath::EdenLegacyDir) / "keys",
        Common::FS::GetLegacyPath(Common::FS::EmuPath::YuzuDir) / "keys",
        Common::FS::GetLegacyPath(Common::FS::EmuPath::SudachiDir) / "keys",
        Common::FS::GetLegacyPath(Common::FS::EmuPath::CitronDir) / "keys",
        Common::FS::GetEdenPath(Common::FS::EdenPath::EdenDir) / "keys",
        Common::FS::GetEdenPath(Common::FS::EdenPath::EdenDir) / "sysdata",
        "/storage/emulated/0/STORM SWITCH/keys",
        "/storage/emulated/0/STORM EDEN/keys",
        "/storage/emulated/0/Eden/keys",
        "/storage/emulated/0/yuzu/keys",
        "/storage/emulated/0/Citron/keys",
        "/storage/emulated/0/Sudachi/keys",
        "/storage/emulated/0/Switch/keys",
        "/data/user/0/dev.storm_switch/files/keys",
        "/data/user/0/dev.storm_eden/files/keys",
        "/data/data/dev.storm_switch/files/keys",
        "/data/data/dev.storm_eden/files/keys",
        "/data/user/0/org.yuzu.yuzu_emu/files/keys",
        "L:/CONSOLES/Nintendo Switch/STORM EDEN/user/keys",
        "E:/STORM EDEN/user/keys",
        "E:/STORM EDEN 2/user/keys",
    };

#ifdef _WIN32
    const auto exe_dir = Common::FS::GetExeDirectory();
    if (!exe_dir.empty()) {
        candidate_dirs.push_back(exe_dir / "user" / "keys");
        candidate_dirs.push_back(exe_dir / "keys");
    }
    const auto appdata_dir = Common::FS::GetAppDataRoamingDirectory();
    if (!appdata_dir.empty()) {
        candidate_dirs.push_back(appdata_dir / "storm_eden" / "keys");
        candidate_dirs.push_back(appdata_dir / "eden" / "keys");
        candidate_dirs.push_back(appdata_dir / "yuzu" / "keys");
        candidate_dirs.push_back(appdata_dir / "switch" / "keys");
    }
#else
    const auto home_dir = Common::FS::GetHomeDirectory();
    if (!home_dir.empty()) {
        candidate_dirs.push_back(home_dir / ".switch");
    }
#endif

    dev_mode = Settings::values.use_dev_keys.GetValue();

    for (const auto& dir : candidate_dirs) {
        if (!Common::FS::Exists(dir)) {
            continue;
        }
        if (dev_mode) {
            LoadFromFile(dir / "dev.keys_autogenerated", false);
            LoadFromFile(dir / "dev.keys", false);
        } else {
            LoadFromFile(dir / "prod.keys_autogenerated", false);
            LoadFromFile(dir / "prod.keys", false);
        }
        LoadFromFile(dir / "title.keys_autogenerated", true);
        LoadFromFile(dir / "title.keys", true);
        LoadFromFile(dir / "console.keys_autogenerated", false);
        LoadFromFile(dir / "console.keys", false);
    }
}
