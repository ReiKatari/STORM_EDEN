...
Result NcaFileSystemDriver::OpenStorageImpl(VirtualFile* out, NcaFsHeaderReader* out_header_reader,
    ...
    if (!m_reader->HasFsInfo(fs_index)) {

        R_THROW(ResultPartitionNotFound);
    }

    // NCZ HACK: Register partition mappings with NCZVirtualFile
    auto body_storage = m_reader->GetSharedBodyStorage();
    if (body_storage && body_storage->IsNczFile()) {
        auto* ncz_file = static_cast<NCZVirtualFile*>(body_storage.get());
        if (!ncz_file->partitions_registered) {

            constexpr std::size_t NcaHeaderSize = 0x4000;
            constexpr std::size_t NcaXtsHeaderSize = 0xC00; // Only 0x200-0xC00 is XTS encrypted
            s64 next_physical_offset = 0; // The ZSTD stream starts exactly after the 0x4000 uncompressed header
            s64 max_virtual_end = ncz_file->decompressed_size;
            
            // CRITICAL FIX: ReadDecryptedHeader reads through AesXtsStorage which
            // XTS-decrypts ALL bytes. But only 0x200-0xC00 is XTS-encrypted;
            // bytes 0xC00-0x4000 are AES-CTR encrypted section data.
            // XTS-decrypting them corrupts the data, causing "Invalid Magic" errors.
            //
            // Solution: Read only the 0xC00 NCA header via XTS storage,
            // then read 0xC00-0x4000 directly from the raw underlying file.
            std::vector<u8> hdr_buf(NcaHeaderSize, 0);
            std::size_t read = 0;
            
            if (ncz_file->is_header_uncompressed || ncz_file->is_raw_nca) {
                read = m_reader->ReadDecryptedHeader(hdr_buf.data(), NcaXtsHeaderSize);
                
                // Read the remaining 0xC00-0x4000 bytes from raw file (NOT through XTS)
                if (read >= NcaXtsHeaderSize && ncz_file->file) {
                    std::size_t raw_read = ncz_file->file->Read(
                        hdr_buf.data() + NcaXtsHeaderSize,
                        NcaHeaderSize - NcaXtsHeaderSize,
                        NcaXtsHeaderSize);
                    read = NcaXtsHeaderSize + raw_read;
                }
            } else {
                read = ncz_file->Read(hdr_buf.data(), NcaHeaderSize, 0);
            }
            
            for (s32 i = 0; i < m_reader->GetFsCount(); ++i) {
                if (!m_reader->HasFsInfo(i)) {

                    continue;
                }
                
                NcaFsHeaderReader header_reader;
                auto init_res = header_reader.Initialize(*m_reader, i);

                
                if (init_res.IsSuccess()) {
                    s64 v_offset = GetFsOffset(*m_reader, i);
                    s64 v_end = GetFsEndOffset(*m_reader, i);
                    s64 v_size = v_end - v_offset;
                    
                    if (v_end > max_virtual_end) {
                        max_virtual_end = v_end;
                    }
                    ...
                    s64 p_size = 0;
                     if (header_reader.ExistsSparseLayer()) {
                          ...
                          p_offset = sparse_info.physical_offset;
                          p_size = sparse_info.GetPhysicalSize();
                        ...
                        if (orig_enc_type == NcaFsHeader::EncryptionType::AesCtr || orig_enc_type == NcaFsHeader::EncryptionType::AesCtrSkipLayerHash) {
                            ...
                            if (enc_size > 0 && read >= static_cast<std::size_t>(enc_offset + enc_size)) {
                                ...
                                if (m_reader->HasExternalDecryptionKey()) {
                                    key = static_cast<const u8*>(m_reader->GetExternalDecryptionKey());
                                } else {
                                    key = static_cast<const u8*>(m_reader->GetDecryptionKey(NcaHeader::DecryptionKey_AesCtr));
                                }
                                
                                if (key != nullptr) {
                                    std::array<u8, 16> iv{};
                                    AesCtrStorage::MakeIv(iv.data(), iv.size(), header_reader.GetAesCtrUpperIv().value, enc_offset);
                                    
                                    Core::Crypto::AESCipher<Core::Crypto::Key128> cipher(
                                        *reinterpret_cast<const std::array<u8, 16>*>(key), Core::Crypto::Mode::CTR);
                                    cipher.SetIV(iv);
                                    u32 magic_before = 0;
                                    std::memcpy(&magic_before, hdr_buf.data() + enc_offset, 4);
                                    
                                    std::vector<u8> temp_buf(hdr_buf.data() + enc_offset, hdr_buf.data() + enc_offset + enc_size);
                                    cipher.Transcode(temp_buf.data(), enc_size, temp_buf.data(), Core::Crypto::Op::Decrypt);
                                    
                                    u32 magic_after = 0;
                                    std::memcpy(&magic_after, temp_buf.data(), 4);
                                    
                                    bool should_decrypt_header_partition = !ncz_file->is_raw_nca && ncz_file->HasDecryptedSections() && ncz_file->is_header_uncompressed;
                                    if (should_decrypt_header_partition) {
                                        std::memcpy(hdr_buf.data() + enc_offset, temp_buf.data(), enc_size);
                                    }
                                    
                                    char key_hex[33]{};
                                    std::sprintf(key_hex, "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
                                                 key[0], key[1], key[2], key[3], key[4], key[5], key[6], key[7],
                                                 key[8], key[9], key[10], key[11], key[12], key[13], key[14], key[15]);
                                                 
                                    std::array<u8, 16> rights_id{};
                                    m_reader->GetRightsId(rights_id.data(), rights_id.size());
                                    char rid_hex[33]{};
                                    std::sprintf(rid_hex, "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
                                                 rights_id[0], rights_id[1], rights_id[2], rights_id[3], rights_id[4], rights_id[5], rights_id[6], rights_id[7],
                                                 rights_id[8], rights_id[9], rights_id[10], rights_id[11], rights_id[12], rights_id[13], rights_id[14], rights_id[15]);
                    ...
                    ncz_file->RegisterPartition(i, reg_v_offset, reg_v_size, p_offset, p_size);
                }
            }
            ncz_file->decompressed_size = max_virtual_end;
            
            if (read > 0) {
                ncz_file->SetDecryptedHeader(hdr_buf.data(), read);
            }
            
            ncz_file->partitions_registered = true;
    ...
    // NCZ HACK: NCZ files store already-decrypted data (the NCZSECTN sections
    // describe the encryption that was removed during compression). Skip the
    // AES encryption layer to avoid double-decryption which garbles the data.
    ...
    if (out_header_reader->ExistsSparseLayer()) {
        ...
        if (out_header_reader->ExistsSparseMetaHashLayer() && !is_ncz_body) {
    ...
    if (patch_info.HasAesCtrExTable()) {
        ...
        ASSERT(effective_encryption_type == NcaFsHeader::EncryptionType::None ||
               ...
               effective_encryption_type == NcaFsHeader::EncryptionType::AesCtrExSkipLayerHash);
        ...
        if (aes_ctr_ex_storage_meta_storage == nullptr) {
            ...
            R_TRY(this->CreateAesCtrExStorageMetaStorage(
                ...
                effective_encryption_type, out_header_reader->GetAesCtrUpperIv(),
                patch_info));
        }
        ...
        R_TRY(this->CreateAesCtrExStorage(
            ...
            patch_info, effective_encryption_type));
    ...
    // NCZ bypass: For NCZ files, skip hash verification but still extract the
    // data region. NCZ decompressed data has correct plaintext content but
    // CreateSha256Storage fails due to size mismatches with the hash tree.
    // AES decryption is already skipped for NCZ via is_ncz_body earlier.
    {
        ...
        if (is_ncz_or_cache) {
            ...
            case NcaFsHeader::HashType::HierarchicalSha3256Hash: {
                const auto& hash_data = out_header_reader->GetHashData().hierarchical_sha256_data;
                const auto& data_region = hash_data.hash_layer_region[1];



                *out = std::make_shared<OffsetVfsFile>(std::move(storage),
                                                       data_region.size, data_region.offset);
                R_SUCCEED();
            }
            case NcaFsHeader::HashType::HierarchicalIntegrityHash: {
                const auto& meta_info = out_header_reader->GetHashData().integrity_meta_info;
                const auto max_layers = meta_info.level_hash_info.max_layers;



                if (max_layers >= 2) {
                    const auto& data_layer = meta_info.level_hash_info.info[max_layers - 2];
                    *out = std::make_shared<OffsetVfsFile>(std::move(storage),
                                                           data_layer.size, data_layer.offset);
                } else {
                    *out = std::move(storage);
                }
                R_SUCCEED();
    ...
    // Create the non-raw storage (applies hash verification and extracts data region).
...
Result NcaFileSystemDriver::CreateBodySubStorage(VirtualFile* out, s64 offset, s64 size) {
    ...
    // NCZ HACK: If the underlying storage is an NCZ file, the NCA header's partition
    // offsets may exceed the NCZ's reported decompressed_size (which is the packed block
    // data size). Expand the NCZ's virtual size to match the NCA partition layout.
    auto raw_body = m_reader->GetSharedBodyStorage();
    if (raw_body && raw_body->IsNczFile() && offset + size > body_size) {
        auto* ncz_file = static_cast<NCZVirtualFile*>(raw_body.get());
        ncz_file->decompressed_size = static_cast<std::size_t>(offset + size);
        body_size = offset + size;
        LOG_INFO(Service_FS, "NCZ: Expanded decompressed_size to {} for partition at offset={}, size={}",
                 body_size, offset, size);
    }
...
Result NcaFileSystemDriver::CreateSparseStorageMetaStorage(VirtualFile* out,
    ...
    bool is_ncz = false;
    auto raw_body = m_reader->GetSharedBodyStorage();
    if (raw_body && raw_body->IsNczFile()) {
        auto* ncz_file = static_cast<NCZVirtualFile*>(raw_body.get());
        if (ncz_file->HasDecryptedSections()) {
            is_ncz = true;
        }
...
Result NcaFileSystemDriver::CreateSparseStorage(VirtualFile* out, s64* out_fs_data_offset,
    ...
    if (header.entry_count != 0) {
        ...
        // Create the sparse storage.
        R_TRY(this->CreateSparseStorageCore(std::addressof(sparse_storage), std::move(body_substorage),
...
Result NcaFileSystemDriver::CreateAesCtrExStorage(
    ...
    const NcaAesCtrUpperIv& upper_iv, const NcaPatchInfo& patch_info, NcaFsHeader::EncryptionType encryption_type) {
    // Validate pre-conditions.
    ASSERT(out != nullptr);
    ASSERT(base_storage != nullptr);
    ASSERT(meta_storage != nullptr);
    ...
    if (m_reader->HasExternalDecryptionKey()) {
        ...
        if (encryption_type == NcaFsHeader::EncryptionType::None) {
            R_TRY(AesCtrCounterExtendedStorage::CreateDummyDecryptor(std::addressof(decryptor)));
        } else {
        ...
        if (encryption_type == NcaFsHeader::EncryptionType::None) {
            R_TRY(AesCtrCounterExtendedStorage::CreateDummyDecryptor(std::addressof(sw_decryptor)));
        } else {
