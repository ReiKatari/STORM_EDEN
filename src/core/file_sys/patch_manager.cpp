...
constexpr std::array<const char*, 15> EXEFS_FILE_NAMES{
...
VirtualFile PatchManager::PatchRomFS(const NCA* base_nca, VirtualFile base_romfs,
    ...
    if (!update_disabled && update_raw != nullptr && base_nca != nullptr) {
        ...
        DecompressIfNcz(packed_update_raw);
...
std::vector<Patch> PatchManager::GetPatches(VirtualFile update_raw) const {
    ...
    for (const auto& [slot, entry] : dlc_entries_with_origin) {
        ...
        const bool matches_base = base_tid == title_id || base_tid == GetAOCBaseTitleID(title_id);
        if (!matches_base) {
...
PatchManager::Metadata PatchManager::ParseControlNCA(const NCA& nca) const {
    ...
    if (base_romfs == nullptr) {
        if (std::FILE* dbg = nullptr) {
            std::fprintf(dbg, "[ICON PARSE] FAIL: nca.GetRomFS() returned nullptr! NCA TitleID=%016llX, Type=%d, Status=%d\n",
                         static_cast<unsigned long long>(nca.GetTitleId()),
                         static_cast<int>(nca.GetType()),
    ...
    if (std::FILE* dbg = nullptr) {
        std::fprintf(dbg, "[ICON PARSE] Step1 OK: base_romfs size=%zu, IsNcz=%d\n",
                     base_romfs->GetSize(), base_romfs->IsNczFile());
        std::fflush(dbg); std::fclose(dbg);
    }
    ...
    if (romfs == nullptr) {
        if (std::FILE* dbg = nullptr) {
            std::fprintf(dbg, "[ICON PARSE] FAIL: PatchRomFS returned nullptr!\n");
            std::fflush(dbg); std::fclose(dbg);
        }
        return {};
    }
    if (std::FILE* dbg = nullptr) {
        std::fprintf(dbg, "[ICON PARSE] Step2 OK: patched romfs size=%zu\n", romfs->GetSize());
        std::fflush(dbg); std::fclose(dbg);
    }
    ...
    if (extracted == nullptr) {
        if (std::FILE* dbg = nullptr) {
            std::fprintf(dbg, "[ICON PARSE] FAIL: ExtractRomFS returned nullptr! romfs_size=%zu\n", romfs->GetSize());
            // Dump first 32 bytes of romfs to check if it's valid
            std::vector<u8> hdr(32);
            auto read = romfs->Read(hdr.data(), 32, 0);
            std::fprintf(dbg, "[ICON PARSE] RomFS first 32 bytes (read=%zu): ", read);
            for (size_t i = 0; i < read && i < 32; i++)
                std::fprintf(dbg, "%02X ", hdr[i]);
            std::fprintf(dbg, "\n");
            std::fflush(dbg); std::fclose(dbg);
        }
        return {};
    ...
    if (std::FILE* dbg = nullptr) {
        std::fprintf(dbg, "[ICON PARSE] Step3 OK: ExtractRomFS success, files:\n");
        for (const auto& f : extracted->GetFiles())
            std::fprintf(dbg, "[ICON PARSE]   file: %s (size=%zu)\n", f->GetName().c_str(), f->GetSize());
        std::fflush(dbg); std::fclose(dbg);
    }
    ...
    for (const auto& language : priority_language_names) {
        ...
        if (icon_file != nullptr) {
            if (std::FILE* dbg = nullptr) {
                std::fprintf(dbg, "[ICON PARSE] Found icon: icon_%s.dat (size=%zu)\n",
                             std::string(language).c_str(), icon_file->GetSize());
                std::fflush(dbg); std::fclose(dbg);
            }
            break;
        }
    ...
    if (icon_file == nullptr) {
        if (std::FILE* dbg = nullptr) {
            std::fprintf(dbg, "[ICON PARSE] WARNING: No icon_*.dat found in any language!\n");
            std::fflush(dbg); std::fclose(dbg);
        }
    }
