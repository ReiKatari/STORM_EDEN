...
protected:
    ...
    void RegisterHandlers(const FunctionInfoTyped<T>* functions, std::size_t n) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            this->handlers.emplace_hint(this->handlers.cend(), functions[i].expected_header, functions[i]);
        }
    }
    ...
    void RegisterHandlersTipc(const FunctionInfoTyped<T>* functions, std::size_t n) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            this->handlers_tipc.emplace_hint(this->handlers_tipc.cend(), functions[i].expected_header, functions[i]);
        }
    }
