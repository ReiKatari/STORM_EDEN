...
private:
    ...
    struct FunctionInfoBase {
        u32 expected_header{};
        std::function<void(HLERequestContext&)> handler{};
...
protected:
    ...
    struct FunctionInfoTyped {
        ...
        constexpr FunctionInfoTyped(u32 expected_header_, HandlerFnP<T> handler_callback_, const char* name_)
            : expected_header{expected_header_}, handler_callback{handler_callback_}, name{name_} {}
    };
    ...
    void RegisterHandlers(const FunctionInfoTyped<T>* functions, std::size_t n) {
        this->handlers.reserve(this->handlers.size() + n);
        for (std::size_t i = 0; i < n; ++i) {
            auto callback = functions[i].handler_callback;
            std::function<void(HLERequestContext&)> fn;
            if (callback != nullptr) {
                fn = [this, callback](HLERequestContext& ctx) {
                    (static_cast<T*>(this)->*callback)(ctx);
                };
            }
            this->handlers.insert_or_assign(functions[i].expected_header, FunctionInfoBase{
                functions[i].expected_header,
                std::move(fn),
                functions[i].name,
            });
        }
    ...
    void RegisterHandlersTipc(const FunctionInfoTyped<T>* functions, std::size_t n) {
        this->handlers_tipc.reserve(this->handlers_tipc.size() + n);
        for (std::size_t i = 0; i < n; ++i) {
            auto callback = functions[i].handler_callback;
            std::function<void(HLERequestContext&)> fn;
            if (callback != nullptr) {
            ...
            this->handlers_tipc.insert_or_assign(functions[i].expected_header, FunctionInfoBase{
                functions[i].expected_header,
                std::move(fn),
                functions[i].name,
            });
