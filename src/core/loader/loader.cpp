...
bool IsContainerType(FileType type) {
    return type == FileType::NSP || type == FileType::NSZ || type == FileType::XCI || type == FileType::XCZ;
}
...
std::unique_ptr<AppLoader> GetLoader(Core::System& system, FileSys::VirtualFile file,
    ...
    if (type != filename_type && !(file->GetName() == "00" && type == FileType::NAX)) {
        LOG_WARNING(Loader, "File {} has a different type ({}) than its extension ({}).",
                    file->GetName(), GetFileTypeString(type), GetFileTypeString(filename_type));
        if (FileType::Unknown == type) {
            type = filename_type;
            LOG_INFO(Loader, "Using extension-based type {} for file {}.",
                     GetFileTypeString(type), file->GetName());
        }
    }

    if (type == FileType::Unknown || type == FileType::Error) {
        LOG_ERROR(Loader, "Unable to identify file type for {}. Content detection: {}, Extension: {}.",
                  file->GetName(), GetFileTypeString(type), GetFileTypeString(filename_type));
        return nullptr;
    }
