...
FileType AppLoader_XCI::IdentifyType(const FileSys::VirtualFile& xci_file) {
    ...
    const auto extension = Common::ToLower(std::string(Common::FS::GetExtensionFromFilename(xci_file->GetName())));
    const auto file_type = (extension == "xcz") ? FileType::XCZ : FileType::XCI;
...
AppLoader_XCI::LoadResult AppLoader_XCI::Load(Kernel::KProcess& process, Core::System& system) {
    ...
    if (xci->GetStatus() != ResultStatus::Success) {
        LOG_ERROR(Loader, "XCI/XCZ {} container parse failed: {}.", file->GetName(), GetResultStatusString(xci->GetStatus()));
        return {xci->GetStatus(), {}};
    }

    if (xci->GetProgramNCAStatus() != ResultStatus::Success) {
        LOG_ERROR(Loader, "XCI/XCZ {} program NCA status error: {}.", file->GetName(), GetResultStatusString(xci->GetProgramNCAStatus()));
    ...
    if (result.first != ResultStatus::Success) {
        LOG_ERROR(Loader, "XCI/XCZ {} NCA loader failed: {}.", file->GetName(), GetResultStatusString(result.first));
        return result;
    }
