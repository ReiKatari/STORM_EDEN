...
class AddonsDelegate : public QStyledItemDelegate {
public:
    explicit AddonsDelegate(QObject* parent = nullptr) : QStyledItemDelegate(parent) {}

    void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const override {
        QStyleOptionViewItem opt = option;
        initStyleOption(&opt, index);

        // Draw default item contents
        QStyledItemDelegate::paint(painter, opt, index);

        // Draw the right, bottom, and left border lines using the system palette mid color
        painter->setPen(opt.palette.color(QPalette::Mid));
        painter->drawLine(opt.rect.bottomLeft(), opt.rect.bottomRight());
        painter->drawLine(opt.rect.topRight(), opt.rect.bottomRight());
        painter->drawLine(opt.rect.topLeft(), opt.rect.bottomLeft());

        // Draw top line for the first row to separate from header
        if (index.row() == 0) {
...
ConfigurePerGameAddons::ConfigurePerGameAddons(Core::System& system_, QWidget* parent)
    ...
    QFont header_font = tree_view->header()->font();
    header_font.setBold(true);
    item_model->setHeaderData(0, Qt::Horizontal, header_font, Qt::FontRole);
    item_model->setHeaderData(1, Qt::Horizontal, header_font, Qt::FontRole);

    item_model->setHeaderData(0, Qt::Horizontal, Qt::AlignCenter, Qt::TextAlignmentRole);
    item_model->setHeaderData(1, Qt::Horizontal, Qt::AlignCenter, Qt::TextAlignmentRole);

    tree_view->header()->setStretchLastSection(false);
    tree_view->header()->setSectionResizeMode(0, QHeaderView::ResizeMode::Stretch);
    tree_view->header()->setMinimumSectionSize(150);

    tree_view->setItemDelegate(new AddonsDelegate(tree_view));
    ...
    connect(&addons_watcher, &QFutureWatcher<AsyncAddonsResult>::finished, this, &ConfigurePerGameAddons::AsyncAddonsLoaded);
}
...
void ConfigurePerGameAddons::LoadConfiguration() {
    ...
    addons_watcher.setFuture(QtConcurrent::run([tid, sys, file_copy]() -> AsyncAddonsResult {
        AsyncAddonsResult result;
        ...
        result.patches = pm.GetPatches(update_raw);
        return result;
    }));
}

void ConfigurePerGameAddons::AsyncAddonsLoaded() {
    const auto result = addons_watcher.result();
    ...
    for (const auto& patch : result.patches) {
        ...
        list_items.back()[0]->setTextAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        list_items.back()[1]->setTextAlignment(Qt::AlignCenter);
