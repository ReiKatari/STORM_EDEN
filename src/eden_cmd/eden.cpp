...
#include "eden_cmd/emu_window/emu_window_sdl3.h"
#ifdef HAS_OPENGL
#include "eden_cmd/emu_window/emu_window_sdl3_gl.h"
#endif
#include "eden_cmd/emu_window/emu_window_sdl3_null.h"
#include "eden_cmd/emu_window/emu_window_sdl3_vk.h"
...
int main(int argc, char** argv) {
    ...
    debug_file << "Checkpoint 0.1: CoInitialize done\n";
    debug_file.flush();
    if (AttachConsole(ATTACH_PARENT_PROCESS)) {
        debug_file << "Checkpoint 0.2: AttachConsole true, doing freopen\n";
        debug_file.flush();
        freopen("CONOUT$", "wb", stdout);
        freopen("CONOUT$", "wb", stderr);
    } else {
        debug_file << "Checkpoint 0.2: AttachConsole false\n";
        debug_file.flush();
    }
#endif

    debug_file << "Checkpoint 0.3: Initializing Logger\n";
    debug_file.flush();
    Common::Log::Initialize();
    debug_file << "Checkpoint 0.4: Setting console color backend\n";
    debug_file.flush();
    Common::Log::SetColorConsoleBackendEnabled(true);
    debug_file << "Checkpoint 0.5: Starting Logger\n";
    debug_file.flush();
    Common::Log::Start();
    debug_file << "Checkpoint 0.6: DetachedTasks initialization\n";
    debug_file.flush();
    Common::DetachedTasks detached_tasks;

    int option_index = 0;
#ifdef _WIN32
    debug_file << "Checkpoint 0.7: Getting CommandLineToArgvW\n";
    debug_file.flush();
    int argc_w;
    auto argv_w = CommandLineToArgvW(GetCommandLineW(), &argc_w);

    if (argv_w == nullptr) {
        debug_file << "Checkpoint 0.8: argv_w is NULL!\n";
        debug_file.flush();
    ...
    debug_file << "Checkpoint 1: Arg parsing done. Filepath: " << filepath << "\n";
    debug_file.flush();

    debug_file << "Checkpoint 1.2: Before try block\n";
    debug_file.flush();

    std::unique_ptr<SdlConfig> config;
    try {
        debug_file << "Checkpoint 1.5: Calling SdlConfig constructor...\n";
        debug_file.flush();
        config = std::make_unique<SdlConfig>(config_path);
    } catch (const std::exception& e) {
        debug_file << "FATAL ERROR: Exception in SdlConfig creation: " << e.what() << "\n";
        debug_file.flush();
        return -1;
    } catch (...) {
        debug_file << "FATAL ERROR: Unknown exception in SdlConfig creation\n";
        debug_file.flush();
        return -1;
    }

    debug_file << "Checkpoint 2: SdlConfig initialized\n";
    debug_file.flush();
    ...
    LOG_CRITICAL(Frontend, "DEBUG: Checkpoint 1 - After InputSubsystem");

    // Apply the command line arguments
    system.ApplySettings();

    LOG_CRITICAL(Frontend, "DEBUG: Checkpoint 2 - After ApplySettings");
    ...
    LOG_CRITICAL(Frontend, "DEBUG: Checkpoint 3 - After Window Creation");
