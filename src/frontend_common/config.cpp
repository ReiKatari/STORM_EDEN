...
void Config::Initialize(const std::optional<std::string> config_path) {
    debug_file << "Checkpoint ConfigInit 1: Entering Initialize\n";
    debug_file.flush();
    const std::filesystem::path default_sdl_config_path =
        FS::GetEdenPath(FS::EdenPath::ConfigDir) / "sdl2-config.ini";
    debug_file << "Checkpoint ConfigInit 2: Path resolved. Default SDL config: " << FS::PathToUTF8String(default_sdl_config_path) << "\n";
    debug_file.flush();
    config_loc = config_path.value_or(FS::PathToUTF8String(default_sdl_config_path));
    debug_file << "Checkpoint ConfigInit 3: Config loc: " << config_loc << "\n";
    debug_file.flush();
    void(FS::CreateParentDir(config_loc));
    debug_file << "Checkpoint ConfigInit 4: Parent dir created/verified\n";
    debug_file.flush();
    SetUpIni();
    debug_file << "Checkpoint ConfigInit 5: SetUpIni done\n";
    debug_file.flush();
    Reload();
    debug_file << "Checkpoint ConfigInit 6: Reload done\n";
    debug_file.flush();
}

void Config::WriteToIni() const {
    debug_file << "Checkpoint WriteToIni 1: Entering. config_loc=" << config_loc << "\n";
    debug_file << "Checkpoint WriteToIni 2: config address=" << config.get() << "\n";
    debug_file.flush();
    ...
    debug_file << "Checkpoint WriteToIni 3: config_type=" << config_type << "\n";
    debug_file.flush();
    FILE* fp = nullptr;
#ifdef _WIN32
    debug_file << "Checkpoint WriteToIni 4: Opening file (Win32)\n";
    debug_file.flush();
    fp = _wfopen(Common::UTF8ToUTF16W(config_loc).data(), L"wb");
#else
    debug_file << "Checkpoint WriteToIni 4: Opening file (non-Win32)\n";
    debug_file.flush();
    fp = fopen(config_loc.c_str(), "wb");
#endif

    debug_file << "Checkpoint WriteToIni 5: File opened. fp=" << fp << "\n";
    debug_file.flush();
    if (fp == nullptr) {
    ...
    debug_file << "Checkpoint WriteToIni 6: Saving config...\n";
    debug_file.flush();
    if (!config) {
        debug_file << "Checkpoint WriteToIni ERROR: config is NULL!\n";
        debug_file.flush();
        fclose(fp);
        return;
    }
    const SI_Error rc = config->Save(writer, false);
    debug_file << "Checkpoint WriteToIni 7: Save completed. rc=" << rc << "\n";
    debug_file.flush();
    if (rc < 0) {
        LOG_ERROR(Frontend, "Config file could not be saved!");
    }
    fclose(fp);
    debug_file << "Checkpoint WriteToIni 8: File closed\n";
    debug_file.flush();
}
...
void Config::ReadMotionTouchValues() {
    ...
    const u32 max_index = static_cast<u32>(std::max(0, num_touch_from_button_maps - 1));
    Settings::values.touch_from_button_map_index = std::clamp(
        Settings::values.touch_from_button_map_index.GetValue(), 0u, max_index);
}
...
void Config::ReadValues() {
    debug_file << "Checkpoint ReadValues: entering\n"; debug_file.flush();
    if (global) {
        debug_file << "Checkpoint ReadValues: calling ReadDataStorageValues\n"; debug_file.flush();
        ReadDataStorageValues();
        debug_file << "Checkpoint ReadValues: calling ReadDebuggingValues\n"; debug_file.flush();
        ReadDebuggingValues();
        debug_file << "Checkpoint ReadValues: calling ReadDisabledAddOnValues\n"; debug_file.flush();
        ReadDisabledAddOnValues();
        debug_file << "Checkpoint ReadValues: calling ReadServiceValues\n"; debug_file.flush();
        ReadServiceValues();
        debug_file << "Checkpoint ReadValues: calling ReadWebServiceValues\n"; debug_file.flush();
        ReadWebServiceValues();
        debug_file << "Checkpoint ReadValues: calling ReadMiscellaneousValues\n"; debug_file.flush();
        ReadMiscellaneousValues();
    }
    debug_file << "Checkpoint ReadValues: calling ReadLibraryAppletValues\n"; debug_file.flush();
    ReadLibraryAppletValues();
    debug_file << "Checkpoint ReadValues: calling ReadNetworkValues\n"; debug_file.flush();
    ReadNetworkValues();
    debug_file << "Checkpoint ReadValues: calling ReadControlValues\n"; debug_file.flush();
    ReadControlValues();
    debug_file << "Checkpoint ReadValues: calling ReadCoreValues\n"; debug_file.flush();
    ReadCoreValues();
    debug_file << "Checkpoint ReadValues: calling ReadCpuValues\n"; debug_file.flush();
    ReadCpuValues();
    debug_file << "Checkpoint ReadValues: calling ReadRendererValues\n"; debug_file.flush();
    ReadRendererValues();
    debug_file << "Checkpoint ReadValues: calling ReadAudioValues\n"; debug_file.flush();
    ReadAudioValues();
    debug_file << "Checkpoint ReadValues: calling ReadSystemValues\n"; debug_file.flush();
    ReadSystemValues();
    debug_file << "Checkpoint ReadValues: done\n"; debug_file.flush();
}
...
void Config::SaveValues() {
    debug_file << "Checkpoint SaveValues 1: Entering SaveValues\n";
    debug_file.flush();
    if (global) {
    ...
    debug_file << "Checkpoint SaveValues 2: Core settings starting\n";
    debug_file.flush();
    SaveLibraryAppletValues();
    SaveNetworkValues();
    debug_file << "Checkpoint SaveValues 3: SaveControlValues starting\n";
    debug_file.flush();
    SaveControlValues();
    debug_file << "Checkpoint SaveValues 4: SaveCoreValues starting\n";
    debug_file.flush();
    SaveCoreValues();
    SaveCpuValues();
    SaveRendererValues();
    SaveAudioValues();
    SaveSystemValues();
    debug_file << "Checkpoint SaveValues 5: WriteToIni starting\n";
    debug_file.flush();

    WriteToIni();
    debug_file << "Checkpoint SaveValues 6: WriteToIni done\n";
    debug_file.flush();
}
...
void Config::SaveControlValues() {
    debug_file << "Checkpoint SaveControlValues 1: Entering\n";
    debug_file.flush();
    BeginGroup(Settings::TranslateCategory(Settings::Category::Controls));

    debug_file << "Checkpoint SaveControlValues 2: BeginGroup done\n";
    debug_file.flush();
    WriteCategory(Settings::Category::Controls);

    debug_file << "Checkpoint SaveControlValues 3: WriteCategory done\n";
    debug_file.flush();
    Settings::values.players.SetGlobal(!IsCustomConfig());
    debug_file << "Checkpoint SaveControlValues 4: SetGlobal done. Players count: " << Settings::values.players.GetValue().size() << "\n";
    debug_file.flush();
    for (std::size_t p = 0; p < Settings::values.players.GetValue().size(); ++p) {
        debug_file << "Checkpoint SaveControlValues 5: Saving player " << p << " starting\n";
        debug_file.flush();
        SavePlayerValues(p);
        debug_file << "Checkpoint SaveControlValues 6: Saving player " << p << " done\n";
        debug_file.flush();
    }
    if (IsCustomConfig()) {
        EndGroup();
    ...
    debug_file << "Checkpoint SaveControlValues 7: SaveTouchscreenValues starting\n";
    debug_file.flush();
    SaveTouchscreenValues();
    debug_file << "Checkpoint SaveControlValues 8: SaveMotionTouchValues starting\n";
    debug_file.flush();
...
void Config::Reload() {
    debug_file << "Checkpoint Config::Reload 1: Entering Reload\n";
    debug_file.flush();
    ReadValues();
    debug_file << "Checkpoint Config::Reload 2: ReadValues done\n";
    debug_file.flush();
    SaveValues();
    debug_file << "Checkpoint Config::Reload 3: SaveValues done\n";
    debug_file.flush();
}
...
void Config::WriteCategory(const Settings::Category category) {
    debug_file << "Checkpoint WriteCategory 1: Entering for category " << static_cast<int>(category) << "\n";
    debug_file.flush();
    const auto& settings = FindRelevantList(category);
    debug_file << "Checkpoint WriteCategory 2: Settings list size: " << settings.size() << "\n";
    debug_file.flush();
    std::ranges::for_each(settings, [&](const auto& setting) { 
        debug_file << "Checkpoint WriteCategory 3: Setting label: " << (setting ? setting->GetLabel() : "NULL") << "\n";
        debug_file.flush();
        WriteSettingGeneric(setting); 
    });
    debug_file << "Checkpoint WriteCategory 4: Done for category " << static_cast<int>(category) << "\n";
    debug_file.flush();
}
...
void Config::WriteSettingGeneric(const Settings::BasicSetting* const setting) {
    if (!setting) {
        debug_file << "Checkpoint WriteSettingGeneric: setting is NULL\n";
        debug_file.flush();
        return;
    }
    debug_file << "Checkpoint WriteSettingGeneric 1: Label=" << setting->GetLabel() << "\n";
    debug_file.flush();
    if (!setting->Save()) {
        return;
    }
    ...
    debug_file << "Checkpoint WriteSettingGeneric 2: key=" << key << "\n";
    debug_file.flush();
    ...
    debug_file << "Checkpoint WriteSettingGeneric 3: done\n";
    debug_file.flush();
}
