...
VirtualAmiibo::Info VirtualAmiibo::LoadAmiibo(const std::string& filename) {
    ...
    if (filename.starts_with("http://") || filename.starts_with("https://")) {
        ...
        size_t host_start = filename.find("://") + 3;
        size_t path_start = filename.find('/', host_start);
        if (path_start != std::string::npos) {
            host = filename.substr(0, path_start);
            path = filename.substr(path_start);
        } else {
            host = filename;
            path = "/";
        }
        ...
        if (res && res->status == 200) {
            ...
            file_path = filename; // Save the URL
