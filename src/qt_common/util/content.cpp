...
void InstallFirmware(const QString& location, bool recursive) {
    ...
    QString progressTitle = is_russian ? QStringLiteral("Установка прошивки...") : tr("Installing Firmware...");
    QString cancelText = is_russian ? QStringLiteral("Отмена") : tr("Cancel");
    
    auto progress =
        QtCommon::Frontend::newProgressDialog(progressTitle, cancelText, 0, 100);
    progress->show();
    ...
    QString failedTitle = is_russian ? QStringLiteral("Ошибка установки прошивки") : tr("Firmware Install Failed");
    QString successTitle = is_russian ? QStringLiteral("Установка прошивки успешно завершена") : tr("Firmware Install Succeeded");
        ...
        if (is_russian) {
            switch (result) {
                case FirmwareInstallResult::Success:
                    msg = QStringLiteral("Успешно установлена прошивка версии %1");
                    break;
                case FirmwareInstallResult::NoNCAs:
                    msg = QStringLiteral("Выбранная папка не содержит файлов прошивки (.nca).");
                    break;
                case FirmwareInstallResult::FailedDelete:
                    msg = QStringLiteral("Не удалось удалить старые файлы прошивки из зарегистрированной папки.");
                    break;
                case FirmwareInstallResult::FailedCopy:
                    msg = QStringLiteral("Не удалось скопировать файлы прошивки.");
                    break;
                case FirmwareInstallResult::FailedCorrupted:
                    msg = QStringLiteral("Установка прошивки была прервана или повреждена.");
                    break;
            }
        } else {
            msg = GetFirmwareInstallResultString(result);
        }
        QtCommon::Frontend::ShowMessage(icon, failedTitle, msg);
    };
    ...
    if (results.size() > 0) {
        ...
        QtCommon::Frontend::Critical(
            QStringLiteral("Проверка целостности прошивки завершилась ошибкой!"),
            QStringLiteral("Сбой проверки для следующих файлов:\n\n%1").arg(failed_names));
        return;
    }
    ...
    QString successMsg = is_russian 
        ? QStringLiteral("Успешно установлена прошивка версии %1").arg(QString::fromStdString(display_version))
        : GetFirmwareInstallResultString(result).arg(QString::fromStdString(display_version));
    QtCommon::Frontend::Information(successTitle, successMsg);
}
...
void VerifyGameContents(const std::string& game_path) {
    auto progress =
        QtCommon::Frontend::newProgressDialog(QStringLiteral("Проверка целостности..."), QStringLiteral("Отмена"), 0, 100);
    progress->show();
    ...
    case ContentManager::GameVerificationResult::Success:
        QtCommon::Frontend::Information(rootObject, QStringLiteral("Проверка целостности пройдена успешно!"),
                                        QStringLiteral("Операция успешно завершена."));
        break;
    case ContentManager::GameVerificationResult::Failed:
        QtCommon::Frontend::Critical(rootObject, QStringLiteral("Проверка целостности завершилась ошибкой!"),
                                     QStringLiteral("Содержимое файлов может быть повреждено или отсутствует."));
        break;
    case ContentManager::GameVerificationResult::NotImplemented:
        QtCommon::Frontend::Warning(
            rootObject, QStringLiteral("Не удалось выполнить проверку целостности"),
            QStringLiteral("Установка прошивки отменена, прошивка может быть в плохом состоянии или повреждена. "
               "Невозможно проверить достоверность содержимого файлов."));
    }
}
...
void InstallKeys() {
    ...
    case FirmwareManager::KeyInstallResult::Success:
        QtCommon::Frontend::Information(QCoreApplication::translate("QtCommon::Content", "Decryption Keys install succeeded"), resMsg);
        break;
    default:
        QtCommon::Frontend::Critical(QCoreApplication::translate("QtCommon::Content", "Decryption Keys install failed"), resMsg);
        break;
    }
}
...
void VerifyInstalledContents() {
    ...
    if (result.empty()) {
        QtCommon::Frontend::Information(QStringLiteral("Проверка целостности пройдена успешно!"),
                                        QStringLiteral("Операция успешно завершена."));
    } else {
        const auto failed_names =
            QString::fromStdString(fmt::format("{}", fmt::join(result, "\n")));
        QtCommon::Frontend::Critical(
            QStringLiteral("Проверка целостности завершилась ошибкой!"),
            QStringLiteral("Сбой проверки для следующих файлов:\n\n%1").arg(failed_names));
    }
}
