    ...
    const auto load_compute{[&](std::ifstream& file, FileEnvironment env) {
        ...
        workers.QueueWork([this, key, env_ = std::move(env), &state, &callback]() mutable {
            try {
                ...
                LOG_CRITICAL(Render_Vulkan, "Exception in compute pipeline worker: {}", e.what());
                std::scoped_lock lock{state.mutex};
                ++state.built;
            } catch (...) {
                LOG_CRITICAL(Render_Vulkan, "Unknown Exception in compute pipeline worker!");
                std::scoped_lock lock{state.mutex};
                ++state.built;
            }
        });
    ...
    const auto load_graphics{[&](std::ifstream& file, std::vector<FileEnvironment> envs) {
        ...
        workers.QueueWork([this, key, envs_ = std::move(envs), &state, &callback]() mutable {
            try {
                ...
                LOG_CRITICAL(Render_Vulkan, "Exception in graphics pipeline worker: {}", e.what());
                std::scoped_lock lock{state.mutex};
                ++state.built;
            } catch (...) {
                LOG_CRITICAL(Render_Vulkan, "Unknown Exception in graphics pipeline worker!");
                std::scoped_lock lock{state.mutex};
                ++state.built;
            }
        });
...
std::unique_ptr<GraphicsPipeline> PipelineCache::CreateGraphicsPipeline(
    ...
    std::ofstream os("shader_crash.txt", std::ios::app);
    os << "Shader Exception in graphics pipeline: " << exception.what() << "\n";
    os.flush();
    Common::Log::Stop();
    std::abort();
    return nullptr;
} catch (const std::exception& exception) {
    LOG_CRITICAL(Render_Vulkan, "FATAL EXCEPTION in graphics pipeline: {}", exception.what());
    std::ofstream os("shader_crash.txt", std::ios::app);
    os << "FATAL EXCEPTION in graphics pipeline: " << exception.what() << "\n";
    os.flush();
    Common::Log::Stop();
    std::abort();
    return nullptr;
} catch (...) {
    LOG_CRITICAL(Render_Vulkan, "UNKNOWN FATAL EXCEPTION in graphics pipeline!");
    std::ofstream os("shader_crash.txt", std::ios::app);
    os << "UNKNOWN FATAL EXCEPTION in graphics pipeline!\n";
    os.flush();
    Common::Log::Stop();
    std::abort();
    return nullptr;
...
std::unique_ptr<ComputePipeline> PipelineCache::CreateComputePipeline(
    ...
    LOG_ERROR(Render_Vulkan, "{}", exception.what());
    std::ofstream os("shader_crash.txt", std::ios::app);
    os << "Shader Exception in compute pipeline: " << exception.what() << "\n";
    os.flush();
    Common::Log::Stop();
    std::abort();
    return nullptr;
} catch (const std::exception& exception) {
    LOG_CRITICAL(Render_Vulkan, "FATAL EXCEPTION in compute pipeline: {}", exception.what());
    std::ofstream os("shader_crash.txt", std::ios::app);
    os << "FATAL EXCEPTION in compute pipeline: " << exception.what() << "\n";
    os.flush();
    Common::Log::Stop();
    std::abort();
    return nullptr;
} catch (...) {
    LOG_CRITICAL(Render_Vulkan, "UNKNOWN FATAL EXCEPTION in compute pipeline!");
    std::ofstream os("shader_crash.txt", std::ios::app);
    os << "UNKNOWN FATAL EXCEPTION in compute pipeline!\n";
    os.flush();
    Common::Log::Stop();
    std::abort();
    return nullptr;
