...
void LoadPipelines(
    ...
    LOG_ERROR(Common_Filesystem, "Pipeline cache IO failure: {}", e.what());
    if (!Common::FS::RemoveFile(filename)) {
        LOG_ERROR(Common_Filesystem, "Failed to delete pipeline cache file {}",
                  Common::FS::PathToUTF8String(filename));
    }
} catch (const std::exception& e) {
    LOG_ERROR(Common_Filesystem, "Pipeline cache parsing failure: {}", e.what());
    if (!Common::FS::RemoveFile(filename)) {
        LOG_ERROR(Common_Filesystem, "Failed to delete pipeline cache file {}",
                  Common::FS::PathToUTF8String(filename));
    }
} catch (...) {
    LOG_ERROR(Common_Filesystem, "Pipeline cache unknown failure!");
    if (!Common::FS::RemoveFile(filename)) {
        LOG_ERROR(Common_Filesystem, "Failed to delete pipeline cache file {}",
                  Common::FS::PathToUTF8String(filename));
    }
