...
void CompressBC5(std::span<const uint8_t> data, uint32_t width, uint32_t height, uint32_t depth,
    ...
    CompressBCN<16, false>(data, width, height, depth, output,
                           [](u8* block_output, const u8* block_input, bool any_alpha) {
                               ...
                               // Compress Red channel as BC4 into first 8 bytes
                               for (int i = 0; i < 16; ++i) {
                                   temp_input[i * 4 + 3] = block_input[i * 4 + 0]; // Red to Alpha
                               }
                               stb_compress_bc3_block(temp_output, temp_input, STB_DXT_NORMAL);
                               std::memcpy(block_output, temp_output, 8);
                               
                               // Compress Green channel as BC4 into next 8 bytes
                               for (int i = 0; i < 16; ++i) {
                                   temp_input[i * 4 + 3] = block_input[i * 4 + 1]; // Green to Alpha
                               }
                               stb_compress_bc3_block(temp_output, temp_input, STB_DXT_NORMAL);
                               std::memcpy(block_output + 8, temp_output, 8);
                           });
}
